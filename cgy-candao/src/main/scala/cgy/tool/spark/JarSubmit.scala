package cgy.tool.spark

import org.apache.spark.sql.SparkSession

/*
spark-submit --class bi.special.pension.Dine_Takeout_Statistical \
--executor-cores 2 --num-executors 2 --total-executor-cores 6 \
--driver-memory 5G --executor-memory 5G --master yarn-client \
--packages mysql:mysql-connector-java:5.1.34 \
./BigDataJyj.jar 2021-01-07 dine_takeout_statistical qcrelease

spark-submit --class bi.special.pension.Dine_Takeout_Statistical \
--executor-cores 2 --num-executors 2 --total-executor-cores 6 \
--driver-memory 5G --executor-memory 5G --master yarn-cluster \
--packages mysql:mysql-connector-java:5.1.34 \
/data/pension/jyj_project.jar 2020-10-01 dine_takeout_statistical jyjrelease

  qcrelease(测试环境)    测试环境saas_plus_data_realse库
  qcbeta(预发布YFB)       测试环境saas_plus_data_beta库
  jyjrelease(正式)      吉野家环境data_realse库
