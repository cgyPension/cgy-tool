...
public class DialectRunner implements Serializable {
	 ...
	 * 是否大小写不敏感（默认大小写不敏感）
	 */
	protected boolean caseInsensitive = cn.hutool.db.GlobalDbConfig.caseInsensitive;
	 ...
	 */
	public int[] insert(Connection conn, cn.hutool.db.Entity... records) throws SQLException {
		checkConn(conn);
	 ...
	 * @throws SQLException SQL执行异常
	 */
	public <T> T insert(Connection conn, cn.hutool.db.Entity record, RsHandler<T> generatedKeysHandler) throws SQLException {
		checkConn(conn);
	 ...
	 * @throws SQLException SQL执行异常
	 */
	public int del(Connection conn, cn.hutool.db.Entity where) throws SQLException {
		checkConn(conn);
	 ...
	 * @throws SQLException SQL执行异常
	 */
	public int update(Connection conn, cn.hutool.db.Entity record, cn.hutool.db.Entity where) throws SQLException {
		checkConn(conn);
		if (CollUtil.isEmpty(record)) {
			throw new SQLException("Empty entity provided!");
		}
	 ...
	 * @throws SQLException SQL执行异常
	 */
	public long count(Connection conn, cn.hutool.db.Entity where) throws SQLException {
		checkConn(conn);
	 ...
	 * @since 5.5.3
	 */
	public <T> T page(Connection conn, SqlBuilder sqlBuilder, cn.hutool.db.Page page, RsHandler<T> rsh) throws SQLException {
		checkConn(conn);
		if (null == page) {
