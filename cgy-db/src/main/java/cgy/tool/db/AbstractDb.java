...
public abstract class AbstractDb implements Serializable {
	 ...
	 * @throws cn.hutool.db.DbRuntimeException 不支持事务
	 */
	protected void checkTransactionSupported(Connection conn) throws SQLException, cn.hutool.db.DbRuntimeException {
		if (null == isSupportTransaction) {
			isSupportTransaction = conn.getMetaData().supportsTransactions();
		}
		if (false == isSupportTransaction) {
			throw new cn.hutool.db.DbRuntimeException("Transaction not supported for current database!");
		}
	}
